#!/bin/bash -l
# ponytail-fork of r-universe-org/build-source@1ee4788f93b7a9f2da882e757ceeff34759a7ab5.
# One change: the 100MB source-package size cap is env-driven
# (SOURCE_SIZE_LIMIT_MB, default 100 -- unset behaves exactly like upstream).
# See UPSTREAM.md in this dir for the full diff and the upgrade path.
set -eo pipefail
...
# ponytail-fork: size cap is a policy-driven env var here (default 100,
# same as upstream) instead of a hardcoded 100M -- SPEC-014's whole reason
# to exist is packages too large for r-universe's own fixed limit.
if [ -n "$(find $SOURCEPKG -prune -size +${SOURCE_SIZE_LIMIT_MB:-100}M)" ]; then
  echo "File $SOURCEPKG is larger than ${SOURCE_SIZE_LIMIT_MB:-100} MB. This is not allowed"
  exit 1
fi
